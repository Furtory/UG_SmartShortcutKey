...
GTT:
  ...
  if (NX<LL)and(NY>LT)and(NY<LB) ;左滑
    ...
    TTD:=3
  else if (NX>LL) and (NX<LR) and (NY>LT) and (NY<LB) and (TTD>0) ;没有滑动0 滑动后又取消-3
    TTD:=-3
  ...
  ; ToolTip Time%KeyPressTime%`nTTD%TTD%`nGM%GM%`nMode%Mode%`n过滤器选择中%过滤器选择中%, , , 3
  ; 手势功能提示
  ...
  if (GM>0) and (过滤器选择中!=1) and (TTD>-3) and (TTD<=0)
  {
    if (KeyPressTime<=500) and (快捷键设置=0)
    {
      TTD:=-1
      if (GM=1)
      {
    ...
    else if (KeyPressTime>500) and (快捷键设置=0)
    {
      TTD:=-2
      if (GM=1)
      {
  ...
  else if (GM>0) and (过滤器选择中!=1) and (TTD=-3)
  {
    ToolTip
  }
  ...
  else if (TTD<=0) and (过滤器选择中!=1) ;没有滑动 取消滑动 不显示箭头
    ToolTip, , , ,2
return